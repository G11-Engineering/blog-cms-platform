...
export async function validateAsgardeoToken(idToken: string): Promise<AsgardeoTokenPayload> {
  try {
    ...
    console.log('🔍 DEBUG: Decoded Token Claims:', {
      sub: decoded?.sub,
      email: decoded?.email,
...
export function mapAsgardeoGroupsToRole(groups: string[] = []): string {
  // Log that groups are being ignored
  if (groups.length > 0) {
    console.log('ℹ️ Asgardeo groups detected but ignored for role assignment:', groups);
  }

  // Always return 'reader' for new users
  // Admins can upgrade roles via admin panel
  console.log('✅ Using default role: reader (all roles managed locally)');
  return 'reader';
}
