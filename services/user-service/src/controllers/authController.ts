...
export const asgardeoLogin = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    console.log('🔐 DEBUG: Asgardeo Token Payload:', {
    ...
    if (userResult.rows.length === 0) {
      ...
      console.log('🆕 NEW USER REGISTRATION via Asgardeo SSO');
      console.log('='.repeat(80));
      console.log(`📧 Email:           ${userInfo.email}`);
      console.log(`👤 Name:            ${userInfo.firstName} ${userInfo.lastName}`);
      console.log(`🏷️  Username:        ${userInfo.email.split('@')[0]}`);
      console.log(`🎭 Assigned Role:   ${userInfo.role} (default - admins can upgrade)`);
      console.log(`📝 Registration:    ${new Date().toISOString()}`);
      console.log(`🔐 Auth Method:     Asgardeo SSO (2FA only - roles managed locally)`);
      console.log('='.repeat(80));
      ...
      console.log(`✅ User created successfully in local database with ID: ${user.id}`);
      console.log('='.repeat(80));
      console.log('');
    } else {
      ...
      console.log(`✅ Existing user login: ${user.email} with role: ${user.role} (role managed locally)`);
    ...
    // 9. Log final result
    console.log('✅ DEBUG: Token Exchange Complete:', {
      userId: user.id,
      email: user.email,
      role: user.role,
